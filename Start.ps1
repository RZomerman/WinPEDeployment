...
If no internet is found, it will continue to use the local version of the PrepareDeployment.ps1 and psm1 scripts
...
write-host "         Welcome to the WinPE PREPARATION SCRIPT " -foregroundColor Yellow
Write-Host "      *********************************************" -foregroundColor Yellow
write-host ""
 ...
 Foreach ($line in $ScriptVersion) {
    ...
    return $false
        ...
        If ($version -gt $Localversion) {
            ...
            Copy-item ($env:TEMP + '\' + 'PrepareDeployment.psm1') 'x:\PrepareDeployment.psm1' -force
            Copy-item ($env:TEMP + '\' + 'PrepareDeployment.ps1') 'x:\PrepareDeployment.ps1' -force

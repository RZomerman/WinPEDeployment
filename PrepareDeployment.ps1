...
#System Validation checks
    ...
    $totalMemoryInGB=CheckRam
        if (($totalMemoryInGB -lt 96) -and (!($WIMDeployment)))  {
            Write-AlertMessage -Message "$totalMemoryInGB is not enough memory to run ASDK"
            Exit-PSHostProcess
        }
        else
        {
            Write-LogMessage -Message "System has $totalMemoryInGB of memory installed"
        }

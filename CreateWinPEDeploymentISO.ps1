    <#
    .SYNOPSIS
        Creating a WinPE ISO image to be used for the automated deployment of WIM images

    .DESCRIPTION
        Creates an ISO image to be used for running WinPE with Network, PowerShell and WMI support
    ...
    .PARAMETER NetworkShare (optional)
        The network location to mount
        \\192.168.0.1\share\ 
    ...
    .PARAMETER USBMode
        ...
        -USBMODE $true (automatically set to true if USBDRIVE is specified)
    ...
    .PARAMETER Hold
        Will Hold the script for closing/creating ISO/USB for further changes in the WinPE Image

    .EXAMPLE
        CreateWinPEDeploymentIso.ps1 -TargetDirectory d:\winpe_image
...
If (($NetworkShare) -and ((!($SharePassword) -or (!($ShareUsername))))) {
...
write-host "        Welcome to the winPE BUILDER " -foregroundColor Yellow
Write-Host "      ********************************" -foregroundColor Yellow
...
#    $localversion=$version
    ...
    If (!$Connection) {
        ...
        Write-LogMessage -Message "Internet Available"
    }        
...
If ($NoInternet) {
    If ((!(Test-Path ($env:TEMP + '\' + 'Start.ps1'))) -and (!(Test-Path ($env:TEMP + '\' + 'PrepareDeployment.psm1'))) -and (!(Test-Path ($env:TEMP + '\' + 'PrepareDeployment.ps1'))) -and (!(Test-Path ($env:TEMP + '\' + 'winpe.jpg')))) {
    ...
    $Uri = ($GitHubLocation + 'PrepareDeployment.psm1')
    #write-host $uri
    $OutFile  = ($env:TEMP + '\' + 'PrepareDeployment.psm1')
    DownloadWithRetry -url $uri -downloadLocation $outfile -retries 3


    $Uri = ($GitHubLocation + 'PrepareDeployment.ps1')
    #write-host $uri
    $OutFile  = ($env:TEMP + '\' + 'PrepareDeployment.ps1')
    DownloadWithRetry -url $uri -downloadLocation $outfile -retries 3
...
$BaseCommand='    . x:\PrepareDeployment.ps1'
If ($ShareUsername) {               
...
If ($NetworkShare){
    $NewCommand=($NewCommand + " -NetworkShare " + $NetworkShare)
}
...
If (!($Reuse)) {
    ...
    IsDISMStillRunning
        write-host ""
        Write-LogMessage -Message "Adding Scripting package" -NoNewline $true
    ...
    If (test-path ($TargetDirectory + "\mount\Windows")) {
        ...
        If (test-path ($env:TEMP + '\' + 'PrepareDeployment.ps1')) {
            $target=($TargetDirectory + '\mount\PrepareDeployment.ps1')
                write-host "." -NoNewline
                Copy-Item ($env:TEMP + '\' + 'PrepareDeployment.ps1') $target -Force
            }
        If (test-path ($env:TEMP + '\' + 'PrepareDeployment.psm1')) {
            $target=($TargetDirectory + '\mount\PrepareDeployment.psm1')
            write-host "." -NoNewline
            Copy-Item ($env:TEMP + '\' + 'PrepareDeployment.psm1') $target -Force
        }
...
#Need to pause if $hold is $true
    If ($Hold) {
        $title = 'INFORMATIONAL:'
        $msg   = (' Holding the script, please type yes to continue')

        $yes = New-Object Management.Automation.Host.ChoiceDescription '&Yes'
        $options = [Management.Automation.Host.ChoiceDescription[]]($yes)
        $default = 0  # $yes
...
#BASE IMAGE CREATED: EXPORT     
        $ISO=('MakeWinPEMedia /ISO ' + $TargetDirectory + ' ' + $TargetDirectory + '\WinPE_Deployment.iso')
        ...
        If (test-path "C:\Program Files (x86)\Windows Kits\10\Assessment and Deployment Kit\Deployment Tools\DandISetEnv.bat") {
            ...
            If ($USBMode -eq $false) {    
                ...
                $MonitorredFile=($TargetDirectory + '\WinPE_Deployment.iso')
                If (test-path $MonitorredFile) {
                ...
                If (test-path ($TargetDirectory + '\WinPE_Deployment.iso')) {
                    Write-LogMessage -Message "Iso successfully created"
                    $ISOOutput=($TargetDirectory + '\WinPE_Deployment.iso')
                    Write-Host "      ***********************************************" -foregroundColor Yellow
